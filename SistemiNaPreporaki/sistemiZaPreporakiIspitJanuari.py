...
def sim_cosine(dataset, p1, p2):
    # similarity = (Suma(i do n) Ai * Bi) / sqrt(Suma(i do n)(Ai ^ 2) * sqrt(Suma(i do n)(Bi ^ 2))
    sum_multiply, sum_p1, sum_p2 = 0, 0, 0
    a, b = [], []
    for key, value in dataset[p1].items():
        a.append(value)
    for key, value in dataset[p2].items():
        b.append(value)
    for i in range(min(len(a), len(b))):
        sum_multiply += a[i]*b[i]
        sum_p1 += pow(a[i], 2)
        sum_p2 += pow(b[i], 2)
    if sum_p1 == 0 or sum_p2 == 0:
        return -1
    return sum_multiply / (sqrt(sum_p1) * sqrt(sum_p2))


if __name__ == '__main__':
    film = input()

    inverted_ratings = transform_prefs(ratings)
    print_dict = {}
    for key, value in inverted_ratings.items():
        cosine = round(sim_cosine(inverted_ratings, key, film), 2)
        euclidean = round(sim_distance(inverted_ratings, key, film), 2)
        pearson = round(sim_pearson(inverted_ratings, key, film), 2)
        print_dict.setdefault(key, (cosine, pearson, euclidean))

    print_dict = sorted(print_dict.items(), key=lambda x: x[0])
    for key, value in print_dict:
        if key != film:

...
def split_data(ratings, user, test_users):
    train_data = {}
    for key, value in ratings.items():
        if key == user or key not in test_users:
            train_data.setdefault(key, value)
    return train_data


def create_new_ratings(test_users, train_data):
    new_dict = {}
    for key, value in train_data.items():
        new_dict.setdefault(key, {})
        for key1, value1 in value.items():
            if key1 not in test_users:
                new_dict[key].setdefault(key1, value1)
    return new_dict
...
if __name__ == '__main__':
    ...
    for user in test_users:
        train_data = split_data(ratings, user, test_users)
        recommendations = get_recommendations(train_data, user)
        if len(recommendations) == 0:
            print(f"{user}: no recommendations ...")
            continue
        recommendations_list = []
        for recommendation in recommendations:
            recommendations_list.append(recommendation[1])
        string = f"{user}: "
        for i in range(len(recommendations_list)-1):
            string += f"{recommendations_list[i]}; "
        string += f"{recommendations_list[-1]}"
        print(string)

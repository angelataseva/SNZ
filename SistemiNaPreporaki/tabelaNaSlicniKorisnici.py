...
def tabela_na_slichni_korisnici(reviews):
    ...
    for key in reviews.keys():
        slicnosti.setdefault(key, {})
        for key1 in reviews.keys():
            if key == key1:
                continue
            euclidean = round(sim_distance(reviews, key, key1), 3)
            pearson = round(sim_pearson(reviews, key, key1), 3)
            films = set(reviews[key].keys())
            films1 = set(reviews[key1].keys())
            count = len(films.intersection(films1))
            slicnosti[key].setdefault(key1, (euclidean, pearson, count))
    return slicnosti

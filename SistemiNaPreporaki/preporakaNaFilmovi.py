...
if __name__ == '__main__':
    ...
    euclidean = get_recommendations(ratings, user_name, sim_distance)
    pearson = get_recommendations(ratings, user_name, sim_pearson)

    euclidean = sorted(euclidean, key=lambda x: x[0], reverse=True)[0]
    pearson = sorted(pearson, key=lambda x: x[0], reverse=True)[0]

    if euclidean[1] == pearson[1]:
        print(euclidean[1])
    else:
        inverted_ratings = transform_prefs(ratings)
        item_based = get_recommendations_item_based(inverted_ratings, user_name)[0]

        if item_based[1] == euclidean[1] or item_based[1] == pearson[1]:
            print(item_based[1])
        else:
            list_recommendations = [euclidean, pearson, item_based]
            list_recommendations = sorted(list_recommendations, key=lambda x: x[0], reverse=True)[0][1]

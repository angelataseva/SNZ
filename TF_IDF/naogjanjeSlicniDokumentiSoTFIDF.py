...
if __name__ == '__main__':
    ...
    pearson_documents = rank_documents(text, documents, pearson)

    same = True
    for i in range(4):
        print(f"Rang po slicnost:{i} Cosine-Document #{cosine_documents[i][1]} "
              f"Pearson-Document #{pearson_documents[i][1]}")

        if cosine_documents[i][1] != pearson_documents[i][1]:
            same = False

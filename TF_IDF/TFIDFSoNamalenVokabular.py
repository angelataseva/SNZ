...
def get_vocabulary(documents):
    ...
    word_freq = sorted(df.items(), key=lambda x: x[0])
    word_freq = sorted(word_freq, key=lambda x: x[1], reverse=True)[:100]
    most_freq_words = []
    for item in word_freq:
        most_freq_words.append(item[0])
    new_vocab = set()
    for word in vocab:
...
if __name__ == '__main__':
    ...
    euclidean = rank_documents(text, documents)
    pearson = rank_documents(text, documents, pearson)

    for i in range(5):
        print(f"Rang po slicnost:{i} Cosine-Document #{euclidean[i][1]} Pearso-Document #{pearson[i][1]}")

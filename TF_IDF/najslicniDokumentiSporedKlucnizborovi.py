...
if __name__ == '__main__':
    ...
    test_vector = process_document(test_doc, df, N, vocab)
    ...
    for i, item in enumerate(dataset):
        ...
        for j, word in enumerate(item[:-1]):
            if word > 0:
                keywords.add(vocab[j])
        new_dataset.setdefault(i, keywords)

    test_keywords = set()
    for i, word in enumerate(test_vector):
        if word > 0:
            test_keywords.add(vocab[i])

    for index, keyword_list in new_dataset.items():
        intersection = test_keywords.intersection(keyword_list)
        new_dataset[index] = intersection

    new_dataset = sorted(new_dataset.items(), key=lambda x: len(x[1]), reverse=True)[:4]
    for item in new_dataset:
        string = f"Dokument #{item[0]}: "
        sorted_words = sorted(item[1])

...
if __name__ == '__main__':
    ...
    train_dataset, df, N, vocab = create_dataset(documents, labels)
    tree = build_tree(train_dataset)

    tp, fp, tn, fn = 0, 0, 0, 0
    for item in test_dataset:
        vector = process_document(item[0], df, N, vocab)
        predicted_label = classify(vector, tree)
        predicted_label = sorted(predicted_label.items(), key=lambda x: x[1], reverse=True)[0][0]
        if item[1] == 'spam':
    ...
    vector = process_document(test_comment, df, N, vocab)
    test_label = classify(vector, tree)
    test_label = sorted(test_label.items(), key=lambda x: x[1], reverse=True)[0][0]

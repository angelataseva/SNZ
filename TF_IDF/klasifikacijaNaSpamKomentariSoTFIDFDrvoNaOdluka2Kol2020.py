...
if __name__ == '__main__':
    ...
    tf_idf_dataset, df, N, vocab = create_dataset(documents, labels)
    tree = build_tree(tf_idf_dataset)

    tp, fp, tn, fn = 0, 0, 0, 0
    for item in test_set:
        vector = process_document(item[0], df, N, vocab)
        predicted_class = classify(vector, tree)
        predicted_class = sorted(predicted_class.items(), key=lambda x: x[1], reverse=True)[0][0]
        if item[1] == 'spam':
    ...
    correctness = (tp + tn) / (tp + fp + tn + fn)
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)

    test_vector = process_document(test_comment, df, N, vocab)
    test_class = classify(test_vector, tree)
    test_class = sorted(test_class.items(), key=lambda x: x[1])[0][0]

    print(f"Tochnost: {correctness}\nPreciznost: {precision}\nOdziv: {recall}\n{test_class}")
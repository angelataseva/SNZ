...
if __name__ == '__main__':
    ...
    for i in sentences:
        keywords = {}
        for j, similarity in enumerate(dataset[i][:-1]):
            if similarity >= threshold:
                keywords.setdefault(vocab[j], similarity)
        keywords = sorted(keywords.items(), key=lambda x: x[1], reverse=True)[:5]

        if len(keywords) == 0:
            print(f"{i} -> No keywords ...")
        else:
            string = f"{i} -> "
            for j, item in enumerate(keywords):
                if j == len(keywords) - 1:

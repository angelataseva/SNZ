...
if __name__ == '__main__':
    ...
    similar_documents = rank_documents(comment, documents)[:n]

    count_spam, count_ham, count = 0, 0, 0
    for document in similar_documents:
        if document[0] > 0:
            count +=1
            if dataset[document[1]][1] == 'spam':
                count_spam += 1
            else:
                count_ham += 1
        else:
    ...
    if count_ham > count_spam:
        print(f'Predvidenata klasa e ham so {count_ham}/{count} glasovi.')
    else:
        print(f'Predvidenata klasa e spam so {count_spam}/{count} glasovi.')

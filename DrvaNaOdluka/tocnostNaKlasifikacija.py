...
def calculate_correctness(test_case, tree, indices):

    predicted_class = []
    for item in test_case:
        predicted_class.append(list(classify(item, tree).keys())[0])

    index = 0
    count_correct = 0
    for item in predicted_class:
        if item == train_data[indices[index]][-1]:
            count_correct += 1
        index += 1

    return count_correct / len(test_case)
...
if __name__ == "__main__":
    ...
    for i, item in enumerate(train_data):
        if i in test_indices:
            test_case.append(item[:-1])
        else:
            new_train_set.append(item)

    tree1 = build_tree(new_train_set[:int(len(new_train_set)*0.5)])
    tree2 = build_tree(new_train_set[int(len(new_train_set)*0.5):])

    correctness1 = calculate_correctness(test_case, tree1, test_indices)
    correctness2 = calculate_correctness(test_case, tree2, test_indices)

...
def calculate_correctness(tree, test, true_classes):
    predicted_classes = []
    for item in test:
        predicted_classes.append(sorted(list(classify(item, tree).items()), key=lambda x: x[1], reverse=True)[0][0])

    count_correct = 0
    for i, item in enumerate(predicted_classes):
        if item == true_classes[i]:
            count_correct += 1

    return count_correct / len(test)
...
if __name__ == "__main__":
    ...
    for item in dataset[:index]:
        ...
        for i, column in enumerate(item):
            if i != column_ind:
                train_item.append(column)
        training_set2.append(train_item)

    test_set = []
    true_classes = []
    for item in dataset[index:]:
        test_set.append(item[:-1])
        true_classes.append(item[-1])
    ...
    for item in dataset[index:]:
        ...
        for i, column in enumerate(item):
            if i != column_ind:
                test_item.append(column)
        test_set2.append(test_item[:-1])
        true_classes2.append(test_item[-1])

    tree1 = build_tree(training_set)
    tree2 = build_tree(training_set2)

    correctness1 = calculate_correctness(tree1, test_set, true_classes)
    correctness2 = calculate_correctness(tree2, test_set2, true_classes2)

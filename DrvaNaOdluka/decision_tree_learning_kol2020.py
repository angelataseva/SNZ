...
def calculate_correctness(tree, test_set, true_classes):
    count = 0
    predicted_classes = []
    for item in test_set:
        predicted_classes.append(list(classify(item, tree).keys())[0])

    for i, item in enumerate(true_classes):
        if item == predicted_classes[i]:
            count += 1
...
if __name__ == "__main__":
    ...
    test_dataset = []
    true_classes_dataset = []
    for item in dataset[int(len(dataset)*0.8):]:
        test_dataset.append(item[:-1])
        true_classes_dataset.append(item[-1])
    ...
    for item in dataset[int(len(dataset)*0.8):]:
        ...
        for i, feature in enumerate(item[:-1]):
            if i != column_ind:
                tmp.append(feature)
        test_set.append(tmp)
        true_classes2.append(item[-1])

    tree1 = build_tree(train_dataset)
    tree2 = build_tree(train_set)

    correctness1 = calculate_correctness(tree1, test_dataset, true_classes_dataset)
    correctness2 = calculate_correctness(tree2, test_set, true_classes2)

    print(f'Tochnost so prvoto drvo na odluka: {correctness1}\n'
          f'Tochnost so vtoroto drvo na odluka: {correctness2}')
...
def print_tree(tree, i, indent=''):
    ...
    if tree.results:
        ...
        # Се печати условот
        print(str(tree.col) + ':' + str(tree.value) + '? Level= (' + str(i) + ')')
        # Се печатат True гранките, па False гранките
        i += 1
        print(indent + 'T-> ', end='')
        print_tree(tree.tb, i, indent + '  ')
        print(indent + 'F-> ', end='')
        print_tree(tree.fb, i, indent + '  ')
...
def filter_predicted_classes(predicted_class):
    final_class = {}

    max = sorted(list(predicted_class.values()), reverse=True)[0]
    for key, value in predicted_class.items():
        if value == max:
            final_class.setdefault(key, value)

    final_class = sorted(final_class.items(), key=lambda item: item[1], reverse=True)
...
if __name__ == "__main__":
    ...
    t1 = build_tree(training_data[:int(len(training_data) * 0.5)])
    predicted_class1 = classify(testCase, t1)
    t2 = build_tree(training_data[int(len(training_data) * 0.5):])
    predicted_class2 = classify(testCase, t2)

    predicted_class1 = filter_predicted_classes(predicted_class1)
    predicted_class2 = filter_predicted_classes(predicted_class2)

...
def create_dictionary(predicted_class, dict):
...
if __name__ == '__main__':
    ...
    tree1 = build_tree(train_set1)
    tree2 = build_tree(train_set2)
    tree3 = build_tree(train_set3)

    predicted_class1 = classify(test_case[:-1], tree1)
    predicted_class2 = classify(test_case[:-1], tree2)
    predicted_class3 = classify(test_case[:-1], tree3)

    points = {0: 0, 1: 0, 2: 0}
    points = create_dictionary(predicted_class1, points)
    points = create_dictionary(predicted_class2, points)
    points = create_dictionary(predicted_class3, points)

    print(f'Glasovi: {points}')
    final_class = sorted(points.items(), key=lambda x: x[1], reverse=True)[0][0]
    print(f'Predvidena klasa: {final_class}')

...
def get_words_with_ignore(doc, stop_words=None):
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            if stop_words is not None and word.lower() not in stop_words:
                words.add(word.lower())
    return words
...
if __name__ == "__main__":
    ...
    classifier2 = NaiveBayes(lambda x: get_words_with_ignore(x, stop_words=words_to_ignore))

    for item in train_data:
        classifier1.train(item[0], item[1])
        classifier2.train(item[0], item[1])

    predicted_class1 = classifier1.classify_document(recenica)
    predicted_class2 = classifier2.classify_document(recenica)

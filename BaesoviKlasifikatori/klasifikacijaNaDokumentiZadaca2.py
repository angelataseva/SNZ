...
if __name__ == "__main__":
    ...
    for word in words:
        for category in classifier.categories():
            probability = round(classifier.get_feature_per_category_probability(word, category), 5)
            weigted_probability = round(classifier.weighted_probability(word, category,
                                        classifier.get_feature_per_category_probability),5)
            print(f'{word} {category} {probability} {weigted_probability}')

    predicted_class = classifier.classify_document(recenica)
    probability_of_class = classifier.get_category_probability_for_document(recenica, predicted_class)
    print(f'{predicted_class} {round(log(probability_of_class, 2),5)}')

...
def get_words_to_include(doc, to_include):
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word.lower() in to_include:
            words.add(word.lower())
    return words
...
if __name__ == '__main__':
    ...
    classifier2 = NaiveBayes(lambda x: get_words_to_include(x, to_include = words_to_include))

    for item in data:
        classifier1.train(item[0], item[1])
        classifier2.train(item[0], item[1])

    predicted_class1 = classifier1.classify_document(comment, 'unknown')
    predicted_class2 = classifier2.classify_document(comment, 'unknown')
    print(f'Klasa predvidena so site zborovi: {predicted_class1}\n'
          f'Klasa predvidena so samo kluchni zborovi: {predicted_class2}')
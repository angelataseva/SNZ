...
def get_words_with_emoticons(doc, emoticons):
    words = set()
    for word in doc.split():
        if 2 < len(word) < 20 or word.lower() in emoticons:
            words.add(word.lower())
    return words
...
if __name__ == '__main__':
    ...
    if start_index >= end_index or end_index <= 0 or start_index >= len(data) - 1 or \
            start_index < 0 or end_index > len(data) - 1:
        start_index = 0
        end_index = 3
    ...
    for i, item in enumerate(data):
        if start_index <= i < end_index:
            test_set.append(item[1])
            true_classes.append(item[0])
        else:
            train_set.append(item)

    classifier1 = NaiveBayes(get_words)
    classifier2 = NaiveBayes(lambda x: get_words_with_emoticons(x, emoticons=emoticons))

    for item in train_set:
        classifier1.train(item[1], item[0])
        classifier2.train(item[1], item[0])

    index = 1
    for i, item in enumerate(test_set):
        predicted_class1 = classifier1.classify_document(item)
        predicted_class2 = classifier2.classify_document(item)

        print(f'{index}:\n'
              f'{item}\n'
              f'Vistinska klasa: {true_classes[i]}\n'
              f'Klasa predvidena so Naiven Baes (bez emotikoni): {predicted_class1}\n'
              f'Klasa predvidena so Naiven Baes (so emotikoni): {predicted_class2}')

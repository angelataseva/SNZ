...
def get_words_with_emoticons(doc, include_words):
    words = get_words(doc)
...
if __name__ == '__main__':
    ...
    train_data = data[:sample_ind] + data[sample_ind + 1:]
    test_data = data[sample_ind]

    classifier1 = NaiveBayes(get_words)
    classifier2 = NaiveBayes(lambda x: get_words_with_emoticons(x, include_words=emoticons))

    for item in train_data:
        classifier1.train(item[1], item[0])
        classifier2.train(item[1], item[0])

    predicted_class1 = classifier1.classify_document(test_data[1])
    predicted_class2 = classifier2.classify_document(test_data[1])

    print(f'{test_data[1]}\nVistinska klasa: {test_data[0]}\n'
